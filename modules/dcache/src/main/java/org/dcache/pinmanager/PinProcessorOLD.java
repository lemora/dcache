...
/**
 * Processes pin requests in state READY_TO_PIN in te pins database.
 * <p>
 ...
 */
public class PinProcessorOLD implements CellMessageReceiver, LeaderLatchListener {

    private static final Logger LOGGER = LoggerFactory.getLogger(PinProcessorOLD.class);

    protected boolean _hasHaLeadership = false;

    protected ScheduledThreadPoolExecutor _executor;
    private ScheduledFuture<?> _pinningTask;
    ...
    protected void runPinning() throws InterruptedException {
        // TODO...
    };

    private void schedulePinningTask() {
        _pinningTask = _executor.scheduleWithFixedDelay(() -> {
                  try {
                      runPinning();
    ...
    private void cancelPinningTask() {
        if (_pinningTask != null) {
            _pinningTask.cancel(true);
        }
    }
    ...
    public synchronized void isLeader() {
        ...
        schedulePinningTask();
        // TODO do stuff on becoming leader
