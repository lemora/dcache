...
public class PinManager implements CellMessageReceiver, LeaderLatchListener, CellInfoProvider {
     ...
     * change, these are abandoned and need to be retried.
     */
    ...
    private void schedulePinningTask() {
        _pinProcessor = executor.scheduleWithFixedDelay(() -> {
                  try {
                      pinTask.runPinning();
                  } catch (InterruptedException e) {
                      LOGGER.info("PinProcessor was interrupted");
                  }
    ...
    public void init() {
        ...
        pinTask = new PinProcessor(dao, poolStub, poolMonitor, maxPinsPerRun);
    }
